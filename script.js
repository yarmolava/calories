document.addEventListener("DOMContentLoaded", () => {
  ...
  if (result) {
    ...
    const protein = Math.round(w * 2); // используем верхнюю границу 2 г/кг
    const fat = Math.round(calories * 0.25 / 9); // средняя норма 25%
    const carbs = Math.round((calories - (protein * 4 + fat * 9)) / 4);

    result.innerHTML = `
      <p style="font-family:'Roboto Condensed', sans-serif; font-weight:700; text-align:center; font-size:18px;">
        Твоя норма калорий: ${calories} ккал в день,<br>
        Белки: ${protein} г в сутки, Углеводы: ${carbs} г в сутки, Жиры: ${fat} г в сутки
      </p>
