document.addEventListener("DOMContentLoaded", () => {
  ...
  if(genderCards.length && nextBtn){
    ...
    nextBtn.addEventListener("click",()=>{if(gender) window.location.href="activity.html"});
  }

  /* ====== ACTIVITY ====== */
  const activityItems=document.querySelectorAll(".activity-item");
  const activityNext=document.getElementById("activityNext");
  const manualCheckbox=document.getElementById("manualActivityCheckbox");
  ...
  if(activityItems.length && activityNext){
    ...
    if(manualCheckbox && manualInput){
      manualCheckbox.addEventListener("change",()=>{
        manualInput.style.display=manualCheckbox.checked?"block":"none";
        if(!manualCheckbox.checked) manualInput.value="";
      });
      manualInput.addEventListener("input",()=>{
        const val=parseFloat(manualInput.value.replace(",","."));
        if(!isNaN(val)&&val>0){selectActivity(val,null);activityNext.disabled=false;}
        else{activityNext.disabled=true;}
      });
    }
    activityNext.addEventListener("click",()=>{if(activity) window.location.href="data.html"});
  }
  ...
  if(age && height && weight && dataNext){
    const validate=()=>{
      const valid=age.value>0 && height.value>0 && weight.value>0 && [...goals].some(g=>g.checked);
